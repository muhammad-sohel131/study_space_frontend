...
import { CalendarDays, Clock, MapPin, Armchair, ChevronRight, LogOut, RefreshCw, CheckCircle2, Timer, AlertCircle, Sparkles, CreditCard, XCircle } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const createBookingMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error.response?.errors?.[0]?.message || 'Failed to renew booking');
    }
  });
  ...
  const getBookingStatus = (booking: Booking) => {
    ...
    if (isAfter(now, end)) {
      return { label: 'Finished', color: 'bg-slate-500', icon: <CheckCircle2 className="h-3 w-3" /> };
    }
    ...
    if (isAfter(now, start) && isBefore(now, end)) {
      ...
      if (isExpiringSoon) {
        return { label: 'Expiring Soon', color: 'bg-amber-500', icon: <Timer className="h-3 w-3" />, canRenew: true };
      }
      return { label: 'Running', color: 'bg-emerald-500', icon: <RefreshCw className="h-3 w-3 animate-spin-slow" />, canRenew: true };
    }

    if (isBefore(now, start)) {
      return { label: 'Upcoming', color: 'bg-indigo-500', icon: <Clock className="h-3 w-3" /> };
    }
  ...
  const renewalPrice = useMemo(() => {
    ...
    if (selectedBooking.bookingType === 'hourly') {
      return (selectedBooking.seat?.pricePerHour || 0) * renewalHours;
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-slate-500 mt-2 font-medium">You have {bookings?.filter(b => b.status === 'confirmed' && isBefore(new Date(), new Date(b.endTime))).length || 0} active sessions.</p>
            </div>
            <div className="flex flex-wrap items-center gap-4">
              ...
              <Link href="/centers" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all group">
                Book New Seat
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
            <CalendarDays className="h-5 w-5 text-indigo-500" /> My Study Sessions
          </h2>
        </div>
                    ...
                    {status.isPending && status.minutesLeft > 0 ? (
                      <div className="space-y-3">
                        <Button 
                          ...
                          className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 font-black uppercase tracking-widest text-[11px] shadow-lg shadow-indigo-100"
                        >
                        ...
                        <Button 
                          ...
                          variant="outline"
                          className="w-full h-12 rounded-xl border-2 border-rose-100 text-rose-500 hover:bg-rose-50 font-black uppercase tracking-widest text-[11px]"
                        >
                          Cancel Booking
                        </Button>
                      </div>
                      ...
                      <Button 
                        onClick={() => handleRenewClick(booking)}
                        className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 font-black uppercase tracking-widest text-[11px] shadow-lg shadow-indigo-100 group/btn"
                      >
